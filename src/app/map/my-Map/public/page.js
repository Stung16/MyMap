...
import { node_1, node_2, node_3, edges } from "@/utils/mapUtil";
...
const Public = () => {
  ...
  const dataMap = data?.data?.mindmaps?.filter((item) => item.status === true);
  const pages = useMemo(() => {
    return dataMap ? Math.ceil(dataMap?.length / query.limit) : 0;
  }, [data, query.limit]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between w-full rounded-tl-xl">
        <h3 className="text-2xl">Public Maps</h3>
      ...
      <div className="flex gap-4">
        <CreateDefault onClick={createMindmap1} />
        <ButtonCreatMap
          src={
